...
interface VideoHookCardProps {
  videoSrc?: string;
  onVideoEnded?: () => void;
}

export const VideoHookCard: React.FC<VideoHookCardProps> = ({
  videoSrc = '/video-hook.mp4',
  onVideoEnded,
}) => {
  const videoRef = useRef<HTMLVideoElement>(null);
  // Has the user intentionally clicked to start the pitch with audio?
  const [hasStartedInteractive, setHasStartedInteractive] = useState(false);
  const [isPlaying, setIsPlaying] = useState(true);
  const [isMuted, setIsMuted] = useState(true);
  const [progress, setProgress] = useState(0);

  // Initial ambient loop on mount (silent preview)
  useEffect(() => {
    if (videoRef.current) {
  ...
  const handleStartPitch = () => {
    ...
    if (!hasStartedInteractive) {
      // First intentional user click: start pitch from 0:00 with audio!
      setHasStartedInteractive(true);
      setIsMuted(false);
      videoRef.current.muted = false;
      videoRef.current.loop = false; // No infinite loop so onEnded triggers!
  ...
  const handleTimeUpdate = () => {
    ...
    // Only track progress towards unlock if the user started the interactive session
    if (hasStartedInteractive) {
  ...
  return (
    <div
      ...
      className="w-full max-w-[280px] mx-auto relative rounded-3xl overflow-hidden bg-[#0A0C14] border border-white/[0.12] shadow-[0_12px_40px_rgba(0,0,0,0.8),0_0_30px_rgba(212,175,55,0.08)] cursor-pointer group select-none aspect-[9/13] flex items-center justify-center"
    >
      {/* Video Element */}
      <video
        ref={videoRef}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#030407]/75 via-transparent to-[#030407]/35 pointer-events-none" />

      {/* Floating Audio Controls (Only visible after user has started) */}
      ...
      {!hasStartedInteractive && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/45 backdrop-blur-[2px] z-20 p-4 text-center">
          {/* Animated Gold Pulsing Play Button */}
          <div className="relative mb-3 group-hover:scale-110 transition-transform">
            <div className="absolute -inset-2 rounded-full bg-[#D4AF37]/30 blur-md animate-ping pointer-events-none" />
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#F3E5AB] via-[#D4AF37] to-[#997A15] flex items-center justify-center text-[#030407] shadow-[0_0_35px_rgba(212,175,55,0.8)] border border-white/40">
              <Play className="w-7 h-7 fill-[#030407] text-[#030407] ml-1" />
            </div>
          </div>

          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#030407]/90 border border-[#D4AF37]/40 text-[#F3E5AB] text-[11px] font-mono font-medium shadow-lg">
            <span>Toca para ver con audio</span>
          </div>
        </div>
      )}

      {/* PAUSED STATE OVERLAY (if user pauses mid-pitch) */}
